...
public class SendHisDataThread extends Thread {
	...
	public void run() {
		...
		while (true) {
			try {
				if (DKMsgProtocol.getInstances().isIsconenected() && (new Date().getHours() > 0 && new Date().getHours() < 6)) {
					List<PassVehicleModel> list = null;
					FailDataLast failData = DKServiceUtil.getService().findDataLastOne(5);
					if (failData != null) {
						long startId = failData.getStartId();
						long endId = failData.getEndId();
						if (startId + rows < endId) {
							list = DKServiceUtil.getService().getVehicleAlarmByStartId(startId,   rows, 5);
							startId = startId + rows;
							failData.setStartId(startId);
							DKServiceUtil.getService().updateFailDataLast(failData);
							sendData(list);

						} else {
							if ((startId + rows) >= endId) {
								list = DKServiceUtil.getService().getVehicleAlarmByStartId(startId, rows, 5);
								DKServiceUtil.getService().deleteFailDataLast(failData.getId());
								sendData(list);
							}
						}
