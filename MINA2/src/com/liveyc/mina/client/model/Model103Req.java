package com.liveyc.mina.client.model;

import java.util.List;

import com.liveyc.mina.client.model.base.BaseModel;

public class Model103Req extends BaseModel {
	private String rzbh;
	public String getRzbh() {
		return rzbh;
	}
	public void setRzbh(String rzbh) {
		this.rzbh = rzbh;
	}
	public String getSjcd() {
		return sjcd;
	}
	public void setSjcd(String sjcd) {
		this.sjcd = sjcd;
	}
	public String getCjbh() {
		return cjbh;
	}
	public void setCjbh(String cjbh) {
		this.cjbh = cjbh;
	}
	public String getTpid() {
		return tpid;
	}
	public void setTpid(String tpid) {
		this.tpid = tpid;
	}
	public String getHphm() {
		return hphm;
	}
	public void setHphm(String hphm) {
		this.hphm = hphm;
	}
	public String getHpys() {
		return hpys;
	}
	public void setHpys(String hpys) {
		this.hpys = hpys;
	}
	public String getJgsj() {
		return jgsj;
	}
	public void setJgsj(String jgsj) {
		this.jgsj = jgsj;
	}
	public String getSpd() {
		return spd;
	}
	public void setSpd(String spd) {
		this.spd = spd;
	}
	public String getClsd() {
		return clsd;
	}
	public void setClsd(String clsd) {
		this.clsd = clsd;
	}
	public String getCsys() {
		return csys;
	}
	public void setCsys(String csys) {
		this.csys = csys;
	}
	public String getCcm() {
		return ccm;
	}
	public void setCcm(String ccm) {
		this.ccm = ccm;
	}
	public String getCcyy() {
		return ccyy;
	}
	public void setCcyy(String ccyy) {
		this.ccyy = ccyy;
	}
	public String getSbsj() {
		return sbsj;
	}
	public void setSbsj(String sbsj) {
		this.sbsj = sbsj;
	}
	public String getHpjg() {
		return hpjg;
	}
	public void setHpjg(String hpjg) {
		this.hpjg = hpjg;
	}
	public String getHpsl() {
		return hpsl;
	}
	public void setHpsl(String hpsl) {
		this.hpsl = hpsl;
	}
	public String getWflx() {
		return wflx;
	}
	public void setWflx(String wflx) {
		this.wflx = wflx;
	}
	public String getCpzbx() {
		return cpzbx;
	}
	public void setCpzbx(String cpzbx) {
		this.cpzbx = cpzbx;
	}
	public String getCpzby() {
		return cpzby;
	}
	public void setCpzby(String cpzby) {
		this.cpzby = cpzby;
	}
	public String getCblx() {
		return cblx;
	}
	public void setCblx(String cblx) {
		this.cblx = cblx;
	}
	public String getCx() {
		return cx;
	}
	public void setCx(String cx) {
		this.cx = cx;
	}
	public String getXs() {
		return xs;
	}
	public void setXs(String xs) {
		this.xs = xs;
	}
	public String getHdsj() {
		return hdsj;
	}
	public void setHdsj(String hdsj) {
		this.hdsj = hdsj;
	}
	public String getSjjy() {
		return sjjy;
	}
	public void setSjjy(String sjjy) {
		this.sjjy = sjjy;
	}
	private String sjcd;
	private String cjbh;
	private String tpid;
	private String hphm;
	private String hpys;
	private String jgsj;
	private String spd;
	private String clsd;
	private String csys;
	private String ccm;
	private String ccyy;
	private String sbsj;
	private String hpjg;
	private String hpsl;
	private String wflx;
	private String cpzbx;
	private String cpzby;
	private String cblx;
	private String cx;
	private String xs;
	private String hdsj;
	private String tpsd;
	private String lxsl;
	public String getLxsl() {
		return lxsl;
	}
	public void setLxsl(String lxsl) {
		this.lxsl = lxsl;
	}
	public String getLxfl() {
		return lxfl;
	}
	public void setLxfl(String lxfl) {
		this.lxfl = lxfl;
	}
	private String lxfl;
	public String getTpsd() {
		return tpsd;
	}
	public void setTpsd(String tpsd) {
		this.tpsd = tpsd;
	}
	private String sjjy;
	private String tpsl;
	public String getTpsl() {
		return tpsl;
	}
	public void setTpsl(String tpsl) {
		this.tpsl = tpsl;
	}
	public List<TpObject> getTpList() {
		return tpList;
	}
	public void setTpList(List<TpObject> tpList) {
		this.tpList = tpList;
	}
	private List<TpObject> tpList;

}
